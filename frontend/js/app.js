...
async function fetchTickets() {
    ...
    try {
        const res = await fetch(
            `${API_BASE_URL}/api/tickets/search?${params.toString()}`
        );
...
function renderTicketTable(tickets) {
    ...
    tbody.innerHTML = tickets.map(t => `
        <tr>
            ...
            <td>
                ${escapeHtml(t.customer ? t.customer.name : 'Unknown')}
            </td>
...
async function openDetailModal(ticketId) {
    try {
        ...
        const priorityBadge =
            document.getElementById('detail-priority-badge');
        ...
        document.getElementById('detail-agent').textContent =
            ticket.agent
                ? `${ticket.agent.name} (${ticket.agent.department})`
                : 'Not Assigned';
        ...
        const resBox =
            document.getElementById('resolution-notes-box');
...
function renderWorkflowControls(ticket) {
    const container =
        document.getElementById('workflow-controls');

    container.innerHTML = '';

    const agentOptions = availableAgents.map(a =>
        `<option value="${a.id}"
            ${ticket.agent && ticket.agent.id === a.id ? 'selected' : ''}>
            ${a.name} (${a.department})
        </option>`
    ).join('');
    ...
    if (ticket.status === 'OPEN') {
        ...
        container.innerHTML = `
            <div class="workflow-action-row">
                ...
                <button
                    ...
                    onclick="performAssign(${ticket.id})">
                    Assign Ticket
                </button>
            </div>

            <p style="font-size: 12px; color: var(--text-muted); margin-top: 8px;">
                Assigning an agent transitions status from
                <strong>OPEN</strong> &rarr;
        ...
        container.innerHTML = `
            <div style="display: flex; flex-direction: column; gap: 10px;">
                ...
                <div class="workflow-action-row">
                    <button
                        class="btn btn-primary btn-sm"
                        onclick="performStatusUpdate(${ticket.id}, 'IN_PROGRESS')">
                ...
                <div
                    ...
                    style="border-top: 1px dashed var(--border-color); padding-top: 10px;">
        ...
        container.innerHTML = `
            <div>
                ...
                <textarea
                    id="wf-resolution-notes"
                    rows="3"
                    placeholder="Enter resolution notes / solution details (Required to resolve)...">
                </textarea>

                <button
                    class="btn btn-success btn-sm"
                    onclick="performResolve(${ticket.id})">
                    &#10004; Mark as RESOLVED
                </button>
        ...
        container.innerHTML = `
            <div class="workflow-action-row">
                ...
                <button
                    ...
                    onclick="performClose(${ticket.id})">
                    &#128274; Close Ticket
                </button>
            ...
            <p style="font-size: 12px; color: var(--text-muted); margin-top: 8px;">
                Closing this ticket is irreversible.
        ...
        container.innerHTML = `
            <div style="color: var(--text-muted); font-size: 13px;">
                &#128274; This ticket is <strong>CLOSED</strong>.
                No further modifications or workflow transitions are permitted.
            </div>
        `;
    }
}
...
async function performAssign(ticketId) {
    const agentSelect =
        document.getElementById('wf-agent-select');

    const agentId =
        agentSelect ? agentSelect.value : null;

    if (!agentId) {
        showToast(
            'Please select an agent to assign',
            'error'
        );
        return;
    }
...
async function performResolve(ticketId) {
    const notesInput =
        document.getElementById('wf-resolution-notes');
    ...
    try {
        const res = await fetch(
            `${API_BASE_URL}/api/tickets/${ticketId}/resolve`,
            {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
